...
const controller = {
    ...
    */
    updateItemNameOnKeyUp: function(event) {
        ...
        // get the id  attribute from updateItemInput'd ls element.
    ...
    */
    turnOnUpdatingMode: function(event) {
        ...
        const updateItemInput = event.target.parentNode.querySelector('.update-item-input');
    ...
    displayTodoItems: function() {
        ...
        */
        ...
        model.items.forEach((item,index) =>{
            const itemLi = document.createElement('li') //li是 4个obj的父级: checkbox list 叉button
            itemLi.id = index
            ...
            //add CSS outfit to checkbox:
            toggleItemCheckbox.classList.add('toggle-item-checkbox')
            toggleItemCheckbox.addEventListener('change',controller.toggleItem) //add event linstener
            ...
            //updateItemInput.addEventListener('keyup',controller.updateItemNameOnKeyUp) //单独这样 这个controller 函数要this, 但是我们不知道this是什么 因为已经过去好久了
            updateItemInput.addEventListener('keyup',controller.updateItemNameOnKeyUp.bind(controller)) //把这个函数和 controller 绑定  exicution context
            ...
            //deleteItemButton.addEventListener('click',controller.deleteItem) 发现controller.deleteItem没有event参数 写一个catch event的函数
